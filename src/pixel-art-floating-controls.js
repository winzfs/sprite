(() => {
  ...
  function activePixelView() { return Boolean($('view-pixelArt')?.classList.contains('active')); }
  ...
  function installStyle() {
    ...
    style.textContent = `
      #${TOGGLE_ID} {
        position: fixed; right: 18px; bottom: 18px; z-index: 9998; display: none;
        border: 0; border-radius: 999px; padding: 12px 16px; font-weight: 800;
        color: #fff; background: rgba(17,24,39,.92); box-shadow: 0 12px 34px rgba(0,0,0,.26);
        backdrop-filter: blur(10px);
      }
      body.pixel-art-view-active #${TOGGLE_ID} { display: inline-flex; align-items: center; gap: 8px; }
      #${PANEL_ID} {
        position: fixed; right: 18px; bottom: 74px; z-index: 9999; display: none;
        width: min(390px, calc(100vw - 28px)); max-height: min(76vh, 720px); overflow: auto;
        padding: 14px; border: 1px solid rgba(148,163,184,.35); border-radius: 18px;
        background: rgba(255,255,255,.94); box-shadow: 0 18px 58px rgba(15,23,42,.28); backdrop-filter: blur(14px);
      }
      ...
      @media (max-width:720px) {
        ...
        #${PANEL_ID} { left:10px; right:10px; bottom:66px; width:auto; max-height:64vh; border-radius:20px 20px 14px 14px; }
  ...
  function dispatchNative(element, type) {
  ...
  function setSourceValue(id, value) {
    ...
    dispatchNative(source, 'input');
    dispatchNative(source, 'change');
  }
  ...
  function copySelect(sourceId, floatingId) {
    const source = $(sourceId);
    const target = $(floatingId);
  ...
  function installPanel() {
    if ($(PANEL_ID) || !requiredReady()) return;
    ...
    $('pixelFloatingSmart')?.addEventListener('change', () => setSourceChecked('pixelArtSmartDetail', $('pixelFloatingSmart').checked));
    $('pixelFloatingUltra')?.addEventListener('change', () => setSourceChecked('pixelArtUltraDetail', $('pixelFloatingUltra').checked));
    $('pixelFloatingAuto')?.addEventListener('change', () => setSourceChecked('pixelArtAutoRun', $('pixelFloatingAuto').checked));
    $('pixelArtSmartDetail')?.addEventListener('change', syncFromSource);
    $('pixelArtUltraDetail')?.addEventListener('change', syncFromSource);
    $('pixelArtAutoRun')?.addEventListener('change', syncFromSource);
  ...
  function updateVisibility() {
    document.body.classList.toggle('pixel-art-view-active', activePixelView() && Boolean($(PANEL_ID)));
    if (!activePixelView()) $(PANEL_ID)?.classList.remove('open');
  }
