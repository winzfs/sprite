...
function readNumber(input, fallback) { const value = Number.parseInt(input?.value, 10); return Number.isFinite(value) ? value : fallback; }
...
function setShift(frameId, shift) {
  const normalized = { x: Math.round(shift.x || 0), y: Math.round(shift.y || 0) };
...
function getSourceRect(frame) {
  ...
  const sx = frame.sx + Math.round(shift.x || 0);
  const sy = frame.sy + Math.round(shift.y || 0);
...
function getFrameIntersection(rect) {
  ...
  return { sx: ix, sy: iy, sw: iw, sh: ih, dx: ix - rect.sx, dy: iy - rect.sy };
}
...
function updateInfoDisplays() {
  ...
  if (!ids.length) { if (ui.selectedShift) ui.selectedShift.value = '선택 없음'; if (ui.selectedBox) ui.selectedBox.value = '선택 없음'; return; }
  if (ids.length === 1) {
    const frame = getFrame(ids[0]); if (!frame) return;
    const rect = getSourceRect(frame);
...
function loadImage(file) {
  ...
  image.onload = () => {
    state.image = image; state.imageUrl = url; state.frameShifts.clear(); state.frameSizes.clear();
    autoDetect(); setStatus(`로드 완료: ${file.name} / ${image.naturalWidth}x${image.naturalHeight}`);
  };
  image.onerror = () => setStatus('이미지를 불러오지 못했습니다.');
  image.src = url;
}
function chooseAutoGrid(width, height) {
  const sizes = [32, 48, 64, 96, 128, 24, 16, 256]; let best = null;
  ...
  if (best) return best;
  const cols = Math.max(1, readNumber(ui.cols, 1)); const rows = Math.max(1, readNumber(ui.rows, 1));
...
function autoDetect() {
  ...
  state.frameShifts.clear(); state.frameSizes.clear(); sliceFrames({ keepAdjustments: false, autoFillOutput: true, silent: true });
...
function fitWholeSheet() {
  ...
  const cols = Math.max(1, readNumber(ui.cols, 1)); const rows = Math.max(1, readNumber(ui.rows, 1));
  ui.ox.value = 0; ui.oy.value = 0; ui.gx.value = 0; ui.gy.value = 0;
  ui.fw.value = Math.max(1, Math.floor(state.image.naturalWidth / cols)); ui.fh.value = Math.max(1, Math.floor(state.image.naturalHeight / rows));
  state.frameShifts.clear(); state.frameSizes.clear(); sliceFrames({ keepAdjustments: false, autoFillOutput: true, silent: true });
  setStatus(`전체 균등 적용: ${cols}열 x ${rows}행 / ${ui.fw.value}x${ui.fh.value}px`);
}
function fillOutputWithAllFrames() { state.outputFrameIds = state.frames.map((frame) => frame.id); state.selectedOutputIndex = state.outputFrameIds.length ? 0 : -1; }
function sliceFrames(options = {}) {
  if (!state.image) return setStatus('먼저 이미지를 업로드하세요.');
  stop(); const s = settings(); const frames = []; let id = 0;
  for (let row = 0; row < s.rows; row += 1) for (let col = 0; col < s.cols; col += 1) {
    const sx = s.ox + col * (s.fw + s.gx); const sy = s.oy + row * (s.fh + s.gy);
    if (sx + s.fw <= state.image.naturalWidth && sy + s.fh <= state.image.naturalHeight) frames.push({ id: id++, sx, sy, w: s.fw, h: s.fh });
  }
  state.frames = frames; state.selectedFrameIds.clear();
  if (options.autoFillOutput) fillOutputWithAllFrames();
  else state.outputFrameIds = state.outputFrameIds.filter((frameId) => frameId < frames.length);
...
function previewFrames() {
  ...
  const s = settings(); const frames = []; let id = 0;
  for (let row = 0; row < s.rows; row += 1) for (let col = 0; col < s.cols; col += 1) {
...
function drawSource() {
  ...
  ui.source.width = state.image.naturalWidth * zoom; ui.source.height = state.image.naturalHeight * zoom;
  sourceCtx.imageSmoothingEnabled = false; sourceCtx.clearRect(0, 0, ui.source.width, ui.source.height); sourceCtx.drawImage(state.image, 0, 0, ui.source.width, ui.source.height);
  const frames = state.frames.length ? state.frames : previewFrames(); sourceCtx.font = `${Math.max(10, 10 * zoom)}px sans-serif`;
...
function drawPreviewGuides(width, height, scale) {
  if (ui.showPreviewFrame?.checked) { previewCtx.save(); previewCtx.strokeStyle = 'rgba(255,255,255,.98)'; previewCtx.lineWidth = 1; previewCtx.strokeRect(0.5, 0.5, width * scale - 1, height * scale - 1); previewCtx.strokeStyle = 'rgba(0,0,0,.85)'; previewCtx.setLineDash([4, 3]); previewCtx.strokeRect(2.5, 2.5, width * scale - 5, height * scale - 5); previewCtx.restore(); }
  if (ui.showPreviewCenter?.checked) { const cx = Math.floor((width * scale) / 2) + 0.5, cy = Math.floor((height * scale) / 2) + 0.5; previewCtx.save(); previewCtx.strokeStyle = 'rgba(110,168,254,.95)'; previewCtx.lineWidth = 1; previewCtx.setLineDash([3, 3]); previewCtx.beginPath(); previewCtx.moveTo(cx, 0); previewCtx.lineTo(cx, height * scale); previewCtx.moveTo(0, cy); previewCtx.lineTo(width * scale, cy); previewCtx.stroke(); previewCtx.restore(); }
}
...
function selectFrameId(id, multi) {
  ...
  const frame = getFrame(id); if (frame) drawPreview(frame); renderOutput(); drawSource(); updateInfoDisplays();
}
function renderOutput() {
  ui.output.textContent = '';
  state.outputFrameIds.forEach((id, index) => {
    const frame = getFrame(id); if (!frame) return;
    const card = createCard(frame, index + 1); card.classList.remove('frame-card'); card.classList.add('output-card');
    if (state.selectedFrameIds.has(id)) card.classList.add('selected'); card.draggable = true;
    ...
    card.addEventListener('dragstart', () => { state.dragIndex = index; }); card.addEventListener('dragover', (event) => event.preventDefault()); card.addEventListener('drop', (event) => { event.preventDefault(); moveOutput(state.dragIndex, index); });
    ui.output.append(card);
  });
}
function selectAllOutput() { state.selectedFrameIds = new Set(state.outputFrameIds); const first = getFrame(state.outputFrameIds[0]); if (first) drawPreview(first); renderOutput(); drawSource(); updateInfoDisplays(); setStatus(`${state.selectedFrameIds.size}개 프레임 전체 선택`); }
function clearSelection() { state.selectedFrameIds.clear(); renderOutput(); drawSource(); updateInfoDisplays(); setStatus('선택 해제'); }
function addSelectedToOutput() {
  const ids = getActiveFrameIds();
  if (!ids.length) return setStatus('추가할 프레임을 먼저 선택하세요.');
  state.outputFrameIds.push(...ids.filter((id) => getFrame(id)));
  renderOutput(); setStatus(`${ids.length}개 프레임을 출력 순서에 추가했습니다.`);
}
function addAllToOutput() { state.outputFrameIds.push(...state.frames.map((frame) => frame.id)); renderOutput(); setStatus(`${state.frames.length}개 프레임을 출력 순서에 추가했습니다.`); }
function clearOutput() { state.outputFrameIds = []; state.selectedFrameIds.clear(); state.selectedOutputIndex = -1; renderOutput(); updateInfoDisplays(); setStatus('출력 순서 비움'); }
function removeSelectedOutput() {
  const remove = new Set(getActiveFrameIds());
  if (!remove.size) return setStatus('제거할 프레임을 먼저 선택하세요.');
  state.outputFrameIds = state.outputFrameIds.filter((frameId) => !remove.has(frameId));
  state.selectedFrameIds.clear(); state.selectedOutputIndex = -1; renderOutput(); updateInfoDisplays(); setStatus(`${remove.size}개 프레임 제거`);
}
function nudgeSelectedFrames(dx, dy) {
  ...
  for (const id of ids) {
    ...
    setShift(id, { x: current.x + dx, y: current.y + dy });
  }
...
function resetSelectedBoxSizes() { const ids = getActiveFrameIds(); if (!ids.length) return setStatus('크기를 초기화할 프레임을 먼저 선택하세요.'); for (const id of ids) state.frameSizes.delete(id); const first = getFrame(ids[0]); renderOutput(); drawSource(); drawPreview(first); updateInfoDisplays(); setStatus(`${ids.length}개 프레임 크기 초기화`); }
...
function detectOpaqueBounds(frame) {
  ...
  if (!detected) return null; return { minX, minY, maxX, maxY };
}
function alignSelectedFrames(horizontal, vertical) {
  const ids = getActiveFrameIds(); if (!ids.length) return setStatus('가운데 정렬할 프레임을 먼저 선택하세요.'); let changed = 0;
...
function nudgeSingleFrame(id, dx, dy) { const frame = getFrame(id); if (!frame) return; const current = getShift(id); setShift(id, { x: current.x + dx, y: current.y + dy }); }
...
function exportPng() {
  ...
  const cols = Math.max(1, Math.min(state.outputFrameIds.length, readNumber(ui.exportCols, readNumber(ui.cols, 6)))); const rows = Math.ceil(state.outputFrameIds.length / cols); const s = settings(); const padX = ui.keepSpacing?.checked ? s.ox : 0, padY = ui.keepSpacing?.checked ? s.oy : 0, gapX = ui.keepSpacing?.checked ? s.gx : 0, gapY = ui.keepSpacing?.checked ? s.gy : 0;
  const validFrames = state.outputFrameIds.map((id) => getFrame(id)).filter(Boolean);
  const cellW = Math.max(...validFrames.map((frame) => getSourceRect(frame).w)); const cellH = Math.max(...validFrames.map((frame) => getSourceRect(frame).h));
  ...
  if (!ui.transparentBg?.checked) { ctx.fillStyle = ui.exportBg?.value || '#d9d9d9'; ctx.fillRect(0, 0, canvas.width, canvas.height); }
  state.outputFrameIds.forEach((id, i) => { const frame = getFrame(id); if (!frame) return; const x = padX + (i % cols) * (cellW + gapX), y = padY + Math.floor(i / cols) * (cellH + gapY); drawFrameImage(ctx, frame, x, y, 1); });
...
function bind() {
  ui.file.addEventListener('change', (event) => loadImage(event.target.files[0])); ui.zoom.addEventListener('input', drawSource); ui.showPreviewFrame?.addEventListener('change', () => drawPreview(state.lastPreviewFrame)); ui.showPreviewCenter?.addEventListener('change', () => drawPreview(state.lastPreviewFrame));
  [ui.fw, ui.fh, ui.cols, ui.rows, ui.ox, ui.oy, ui.gx, ui.gy].forEach((input) => input.addEventListener('input', () => sliceFrames({ keepAdjustments: true, autoFillOutput: false })));
  document.addEventListener('click', (event) => {
    ...
    if (id === 'testButton') setStatus('버튼 동작 정상'); if (id === 'autoDetectButton') autoDetect(); if (id === 'fitWholeButton') fitWholeSheet(); if (id === 'sliceButton') sliceFrames({ keepAdjustments: false, autoFillOutput: true }); if (id === 'resetButton') reset();
    if (id === 'selectAllButton') selectAllOutput(); if (id === 'clearSelectionButton') clearSelection(); if (id === 'addSelectedButton') addSelectedToOutput(); if (id === 'addAllButton') addAllToOutput(); if (id === 'clearOutputButton') clearOutput();
    ...
    if (id === 'centerXButton') alignSelectedFrames(true, false); if (id === 'centerYButton') alignSelectedFrames(false, true); if (id === 'centerBothButton') alignSelectedFrames(true, true); if (id === 'playButton') play(); if (id === 'stopButton') { stop(); setStatus('재생 정지'); }
    if (id === 'moveOutputLeftButton') moveOutput(state.selectedOutputIndex, state.selectedOutputIndex - 1); if (id === 'moveOutputRightButton') moveOutput(state.selectedOutputIndex, state.selectedOutputIndex + 1); if (id === 'removeSelectedOutputButton') removeSelectedOutput(); if (id === 'exportButton') exportPng();
  });
...
bind(); updateInfoDisplays(); setStatus('JS 연결 완료. 보정된 프레임 박스가 출력 미리보기와 내보내기에 직접 적용됩니다.');
